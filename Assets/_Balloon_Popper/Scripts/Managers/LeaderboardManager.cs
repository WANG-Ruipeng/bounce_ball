...
namespace ClawbearGames
{
	public class LeaderboardManager : MonoBehaviour
	{
        ...
        /// <summary>
        /// Connecting to Dreamlo services. 
        /// </summary>
        /// <param name="callback"></param>
        /// <returns></returns>
        private IEnumerator CRConnectingDreamloServices(Action<bool> callback)
		{
		...
		/// <summary>
		/// Getting the leaderboard data from Dreamlo services.
		/// </summary>
		/// <param name="callback"></param>
		/// <returns></returns>
		private IEnumerator CRGettingPlayerLeaderboardData(Action<string> callback)
        {
			string requestUrl = GetLeaderboardDataUrl + "/pipe";
			UnityWebRequest unityWebRequest = new UnityWebRequest(requestUrl);
			DownloadHandlerBuffer dH = new DownloadHandlerBuffer();
		...
		{
			StartCoroutine(CRConnectingDreamloServices((isConnected) =>
			{
		...
		public void CheckUsernameExists(string username, Action<bool> callback)
		{
			...
			StartCoroutine(CRGettingPlayerLeaderboardData((stringDatas) =>
			{
		...
		/// </summary>
		/// <param name="callback"></param>
		public void GetPlayerLeaderboardData(Action<List<LeaderboardData>> callback)
		{
			StartCoroutine(CRGettingPlayerLeaderboardData((stringDatas) =>
			{
                if (!stringDatas.Equals(string.Empty))
                {
					...
					string[] rows = stringDatas.Split(new char[] { '\n' }, StringSplitOptions.RemoveEmptyEntries);
					LeaderboardData[] playerLeaderboardDatas = new LeaderboardData[rows.Length];
					for (int i = 0; i < rows.Length; i++)
					{
						string[] chars = rows[i].Split(new char[] { '|' }, StringSplitOptions.None);
					...
					IComparer<LeaderboardData> comparer = new LeaderboardDataComparer();
					...
					else
					{
						...
						List<LeaderboardData> listPlayerData = new List<LeaderboardData>();
                ...
                {
					callback?.Invoke(new List<LeaderboardData>());
				}
			}));
		}
	}
